...
const LoginTicket = require('./loginticket.js');
const noop = require('lodash.noop');
const PemVerifier = require('./../pemverifier.js');
...
class OAuth2Client extends AuthClient {
  constructor (clientId, clientSecret, redirectUri, opt_opts) {
    super(clientId, clientSecret, redirectUri, opt_opts);
   ...
   * @private
   */
  static get GOOGLE_OAUTH2_AUTH_BASE_URL_ () {
   ...
   * @private
   */
  static get GOOGLE_OAUTH2_TOKEN_URL_ () {
   ...
   * @private
   */
  static get GOOGLE_OAUTH2_REVOKE_URL_ () {
   ...
   * @private
   */
  static get GOOGLE_OAUTH2_FEDERATED_SIGNON_CERTS_URL_ () {
   ...
   * @private
   */
  static get MAX_TOKEN_LIFETIME_SECS_ () {
    return 86400;
  }
  ...
  generateAuthUrl (opt_opts) {
    ...
    const rootUrl = this.opts.authBaseUrl ||
   ...
   * @private
   */
  refreshToken_ (refresh_token, opt_callback) {
    const uri = this.opts.tokenUrl || OAuth2Client.GOOGLE_OAUTH2_TOKEN_URL_;
    const values = {
  ...
  refreshAccessToken (callback) {
    ...
    this.refreshToken_(this.credentials.refresh_token, (err, result, response) => {
      if (err) {
  ...
  getAccessToken (callback) {
    ...
    const shouldRefresh = !credentials.access_token || isTokenExpired;
  ...
  getRequestMetadata (opt_uri, metadataCb) {
    ...
    return this.refreshToken_(thisCreds.refresh_token, (err, tokens, response) => {
      if (err) {
   ...
   * @param {function=} opt_callback Optional callback fn.
   */
  revokeToken (token, opt_callback) {
    const params = querystring.stringify({ token: token });
    const uri = `${OAuth2Client.GOOGLE_OAUTH2_REVOKE_URL_}?${params}`;
    this.transporter.request({
      uri: uri,
      json: true
    }, opt_callback);
  ...
  request (opts, callback) {
    ...
    postRequestCb = (err, body, resp) => {
      ...
      if (retry && (statusCode === 401 || statusCode === 403) &&
        ...
        this._postRequest(err, body, resp, callback);
      }
    };
   ...
   * @private
   */
  _postRequest (err, result, response, callback) {
   ...
   */
  verifyIdToken (idToken, audience, callback) {
    if (!idToken || !callback) {
