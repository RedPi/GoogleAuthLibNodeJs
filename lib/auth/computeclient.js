...
class Compute extends OAuth2Client {
  ...
  refreshToken_ (ignored_, opt_callback) {
    const uri = this.opts.tokenUrl || Compute.GOOGLE_OAUTH2_TOKEN_URL_;
   ...
   * @private
   */
  _injectErrorMessage (err, result, response, callback) {
    if (response && response.statusCode) {
      ...
      if (helpfulMessage) {
        if (err && err.message) {
          helpfulMessage = `${helpfulMessage} ${err.message}`;
        }
